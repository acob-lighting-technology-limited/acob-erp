...
interface PptxModeDialogProps {
  ...
  onExport: (mode: WeeklyPptxMode, theme: WeeklyPptxTheme, order: WeeklyDeptOrder) => void
  showOrderStep?: boolean
}

type Step = 1 | 2 | 3

export function PptxModeDialog({ isOpen, onOpenChange, onExport, showOrderStep = true }: PptxModeDialogProps) {
  ...
  const handleThemeSelect = (t: WeeklyPptxTheme) => {
    ...
    if (showOrderStep) {
      ...
      onExport(mode, t, "default")
      setStep(1)
    }
  }

  const handleOrderSelect = (order: WeeklyDeptOrder) => {
    onExport(mode, theme, order)
    setStep(1)
  }
