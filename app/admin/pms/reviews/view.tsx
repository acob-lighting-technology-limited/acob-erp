...
import { CreateReviewDialog } from "@/app/admin/hr/performance/_components/create-review-dialog"
...
export function AdminPmsReviewsPage({
  backLinkHref,
  reviewCycleBasePath,
  ...
  const goToIndividualCycle = useCallback(
    (cycleId: string) => {
      router.push(`${reviewCycleBasePath ?? "/admin/pms/reviews"}/${encodeURIComponent(cycleId)}?mode=individual`)
    },
  ...
  const goDeptCycle = useCallback(
    (cycleId: string, department: string) => {
      router.push(
        `/admin/pms/reviews/${encodeURIComponent(cycleId)}?mode=department&department=${encodeURIComponent(department)}`
      )
    },
    [router]
  )
