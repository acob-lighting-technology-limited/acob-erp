...
export default function AdminPmsCbtExtraQuestionPage() {
  ...
  useEffect(() => {
    if (cycles.length > 0 && !searchParams.get("review_cycle_id") && !searchParams.get("cycleId")) {
      if (activeCycleId) {
        router.replace(`/admin/pms/cbt/extra?review_cycle_id=${encodeURIComponent(activeCycleId)}`, {
          scroll: false,
        })
      }
    }
  ...
  async function handleSubmit(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      router.replace(`/admin/pms/cbt/extra?review_cycle_id=${encodeURIComponent(form.review_cycle_id)}`, {
        scroll: false,
      })
  ...
  return (
    <DataTablePage
      ...
      icon={Brain}
      backLink={{ href: "/admin/pms/cbt", label: "Back to PMS CBT Overview" }}
      actions={
        <div className="flex items-center gap-2">
          ...
          <Link href="/admin/pms/cbt/extra/scores">
