...
import { Satellite, Plus, Upload, AlertCircle, CheckCircle2, Clock, DollarSign } from "lucide-react"
import Link from "next/link"
import type { StarlinkDashboardStats, StarlinkSiteWithPayments } from "@/types/starlink"
import {
...
export default function StarlinkDashboardPage() {
  ...
  const fetchDashboardData = async () => {
    try {
      setLoading(true)
      const [statsRes, sitesRes] = await Promise.all([fetch("/api/starlink/dashboard"), fetch("/api/starlink/sites")])
  ...
  const handleImportExcel = async () => {
    try {
      ...
      if (response.ok) {
        toast.success(`Import completed: ${result.results.success} successful, ${result.results.failed} failed`)
        fetchDashboardData()
      } else {
  ...
  return (
    <PageWrapper maxWidth="full" background="gradient">
      <PageHeader
        title="Starlink Payment Management"
        description="Track payment schedules and manage documents for all Starlink sites"
        icon={Satellite}
        backLink={{ href: "/admin", label: "Back to Admin" }}
        actions={
          <div className="flex gap-2">
            <Button onClick={handleImportExcel} disabled={importing} variant="outline">
      ...
      <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-4">
        <Card>
          <CardContent className="p-4">
            <div className="flex items-center justify-between">
              <div>
                <p className="text-muted-foreground text-xs font-medium">Total Sites</p>
                <p className="text-foreground mt-1 text-2xl font-bold">{stats?.total_sites || 0}</p>
              </div>
        ...
        <Card>
          <CardContent className="p-4">
            <div className="flex items-center justify-between">
              <div>
                <p className="text-muted-foreground text-xs font-medium">Due This Week</p>
                <p className="text-foreground mt-1 text-2xl font-bold">{stats?.payments_due_this_week || 0}</p>
              </div>
          ...
          <CardContent className="p-4">
            <div className="flex items-center justify-between">
              <div>
                ...
                <p className="text-foreground mt-1 text-2xl font-bold">{stats?.pending_payments || 0}</p>
              </div>
        ...
        <Card>
          <CardContent className="p-4">
            <div className="flex items-center justify-between">
              <div>
                <p className="text-muted-foreground text-xs font-medium">Total Pending</p>
          ...
          <CardContent>
            <div className="space-y-2">
              {stats.upcoming_payments.map((payment, index) => (
                <div key={index} className="flex items-center justify-between rounded-lg bg-white p-3 dark:bg-gray-900">
                  ...
                  <div className="flex items-center gap-2">
                    {payment.amount && <span className="text-sm font-medium">{formatCurrency(payment.amount)}</span>}
        ...
        <CardContent>
          {sites.length === 0 ? (
            <div className="py-12 text-center">
              <Satellite className="text-muted-foreground mx-auto h-12 w-12" />
              <p className="text-muted-foreground mt-4">No sites found. Import your Excel data or add a new site.</p>
                ...
                return (
                  <div
                    ...
                    className={`flex items-center justify-between rounded-lg border p-4 transition-all hover:shadow-md ${
                      isUrgent ? "border-orange-200 bg-orange-50 dark:border-orange-900 dark:bg-orange-950/20" : ""
                    }`}
                  >
                    <div className="flex-1">
                      <div className="flex items-center gap-2">
                        <h3 className="text-foreground font-semibold">{site.site_name}</h3>
                      ...
                      <p className="text-muted-foreground text-xs">Serial: {site.serial_number}</p>
                    </div>

                    <div className="flex items-center gap-4">
                      {site.latest_payment && (
                        <div className="text-right">
                          <p className="text-foreground text-sm font-medium">Next Payment</p>
                          <p className="text-muted-foreground text-xs">
                            {new Date(site.latest_payment.next_payment_due).toLocaleDateString()}
                          </p>
                          {daysUntil !== null && daysUntil !== undefined && (
                            <Badge className={`mt-1 ${getUrgencyBadgeColor(daysUntil)}`}>
