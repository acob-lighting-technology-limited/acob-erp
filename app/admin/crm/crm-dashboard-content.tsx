...
interface CRMDashboardContentProps {
  ...
  initialPipelineByStage: Record<string, PipelineStage>
  initialRecentActivities: any[]
  initialUpcomingFollowUps: any[]
}

export function CRMDashboardContent({
  initialMetrics,
  initialPipelineByStage,
  initialRecentActivities,
  initialUpcomingFollowUps,
}: CRMDashboardContentProps) {
  const router = useRouter()
  const supabase = createClient()

  const [metrics, setMetrics] = useState<DashboardMetrics>(initialMetrics)
  const [pipelineByStage, setPipelineByStage] = useState<Record<string, PipelineStage>>(initialPipelineByStage)
  const [recentActivities, setRecentActivities] = useState<any[]>(initialRecentActivities)
  const [upcomingFollowUps, setUpcomingFollowUps] = useState<any[]>(initialUpcomingFollowUps)
  const [isLoading, setIsLoading] = useState(false)

  // Initial data is provided via props from server component
  // loadDashboard available for client-side refresh
