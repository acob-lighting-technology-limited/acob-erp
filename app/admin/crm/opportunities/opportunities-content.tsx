...
export function OpportunitiesContent({ initialOpportunities, initialPipeline }: OpportunitiesContentProps) {
  const router = useRouter()
  const searchParams = useSearchParams()

  const [opportunities, setOpportunities] = useState<CRMOpportunity[]>(initialOpportunities)
  const [pipeline, setPipeline] = useState<CRMPipeline | null>(initialPipeline)
  const [isLoading, setIsLoading] = useState(false)
  const [search, setSearch] = useState("")
  const [statusFilter, setStatusFilter] = useState("open")
  const [view, setView] = useState<"kanban" | "list">("kanban")

  // Refetch when status filter changes (not on initial mount)
  useEffect(() => {
