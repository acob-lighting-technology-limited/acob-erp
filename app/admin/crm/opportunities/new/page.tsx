...
export default function NewOpportunityPage() {
  ...
  const loadData = async () => {
    try {
      const [contactsRes, pipelinesRes, usersRes] = await Promise.all([
      ...
      if (pipelinesData.data?.length > 0) {
        const defaultPipeline = pipelinesData.data.find((p: CRMPipeline) => p.is_default) || pipelinesData.data[0]
        setSelectedPipeline(defaultPipeline)
        setFormData((prev) => ({
          ...prev,
          pipeline_id: defaultPipeline.id,
          stage: defaultPipeline.stages[0]?.name || "New",
        }))
      }
    } catch (error) {
