...
export async function POST(request: NextRequest) {
  try {
    ...
    if (!sites || !Array.isArray(sites)) {
      return NextResponse.json({ error: "Invalid data format. Expected array of sites." }, { status: 400 })
    }
    ...
    for (const siteData of sites) {
      try {
        ...
        const billingPeriodMatch = siteData.billing_period?.match(/(\w+)\s+(\d+)\s*-\s*(\w+)\s+(\d+)/)
        ...
        if (billingPeriodMatch) {
          ...
          const nextPaymentMatch = siteData.next_payment_due?.match(/(\w+)\s+(\d+),\s+(\d+)/)
          if (nextPaymentMatch) {
        ...
        if (billingStart && billingEnd && nextPaymentDue) {
          ...
          if (!existingPayment) {
            const { error: paymentError } = await supabase.from("starlink_payments").insert({
              site_id: siteId,
        ...
        results.failed++
        results.errors.push(`Failed to import ${siteData.site_name}: ${error.message}`)
