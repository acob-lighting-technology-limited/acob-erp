...
-- Staff can view contacts assigned to them
CREATE POLICY "Staff view assigned contacts" ON crm_contacts
  FOR SELECT TO authenticated
...
-- Staff can view opportunities assigned to them
CREATE POLICY "Staff view assigned opportunities" ON crm_opportunities
  FOR SELECT TO authenticated
  USING (assigned_to = auth.uid());

-- Staff+ can create opportunities
...
-- Staff can view their activities
CREATE POLICY "Staff view own activities" ON crm_activities
  FOR SELECT TO authenticated
  USING (
    assigned_to = auth.uid()
    OR created_by = auth.uid()
