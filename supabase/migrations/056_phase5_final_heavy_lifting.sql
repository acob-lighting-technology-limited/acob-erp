...
FOR SELECT TO authenticated 
USING (has_role('staff') OR has_role('visitor') OR has_role('admin'));

-- Table: tariffs
DROP POLICY IF EXISTS "Visitor read tariffs" ON tariffs;
DROP POLICY IF EXISTS "Staff read tariffs" ON tariffs;
...
CREATE POLICY "Staff can update activities" ON crm_activities 
FOR UPDATE TO authenticated 
USING (has_role('staff') OR has_role('admin'));

DROP POLICY IF EXISTS "Users can delete activities" ON crm_activities;
CREATE POLICY "Staff can delete activities" ON crm_activities 
FOR DELETE TO authenticated 
USING (has_role('staff') OR has_role('admin'));

DROP POLICY IF EXISTS "Users can view all activities" ON crm_activities;
CREATE POLICY "Staff can view all activities" ON crm_activities 
FOR SELECT TO authenticated 
USING (has_role('staff') OR has_role('admin'));

-- Table: notifications
DROP POLICY IF EXISTS "System can create notifications" ON notifications;
CREATE POLICY "System and Staff can create notifications" ON notifications 
FOR INSERT TO authenticated 
