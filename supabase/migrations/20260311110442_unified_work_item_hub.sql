...
INSERT INTO public.tasks (
  title, description, status, department, assigned_by,
  category, week_number, year, completed_at, created_at, updated_at,
  source_type, assignment_type, priority
)
SELECT
  ai.title, ai.description, ai.status, ai.department, ai.assigned_by,
  'weekly_action', ai.week_number, ai.year, ai.completed_at, ai.created_at, ai.updated_at,
  'action_item', 'department', 'medium'
...
CREATE OR REPLACE VIEW public.unified_work AS
  SELECT
    t.id, t.work_item_number, t.title, t.description, t.status, t.priority,
    t.department, t.assigned_to, t.assigned_by, t.assignment_type,
    t.due_date, t.started_at, t.completed_at, t.created_at, t.updated_at,
    t.source_type, t.source_id, t.project_id, t.category,
    t.week_number, t.year, t.progress,
    'tasks'::text AS source_table
  FROM public.tasks t
  UNION ALL
  SELECT
    hd.id, hd.ticket_number AS work_item_number, hd.title, hd.description,
    hd.status, hd.priority, hd.service_department AS department,
    hd.assigned_to, hd.assigned_by, 'individual'::text AS assignment_type,
    (hd.sla_target_at)::date AS due_date, hd.started_at,
    hd.resolved_at AS completed_at, hd.created_at, hd.updated_at,
    'help_desk'::text AS source_type, hd.id AS source_id,
    NULL::uuid AS project_id, NULL::text AS category,
    NULL::integer AS week_number, NULL::integer AS year,
    CASE
