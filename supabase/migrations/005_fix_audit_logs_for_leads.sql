-- ============================================
-- Fix Audit Logs RLS Policy for Leads
-- ============================================
-- This migration fixes the audit logs RLS policy to properly show logs for leads
-- It checks both the user performing the action and target users in the lead's departments
  ...
  EXISTS (
    ...
    AND (
      ...
      EXISTS (
        ...
        AND (
          (p.lead_departments IS NOT NULL AND array_length(p.lead_departments, 1) > 0 AND p2.department = ANY(p.lead_departments))
          OR
          (p.lead_departments IS NULL OR array_length(p.lead_departments, 1) = 0 AND p2.department = p.department)
        )
      )
      OR
