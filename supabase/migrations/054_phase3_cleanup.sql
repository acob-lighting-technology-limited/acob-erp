...
-- Merging "Staff read token sales" and "Visitor read token sales" into a single efficient policy
DROP POLICY IF EXISTS "Staff read token sales" ON token_sales;
...
USING (has_role('staff') OR has_role('visitor') OR has_role('admin') OR has_role('super_admin'));

-- 3. Cleanup overlapping policies on meters (optional but recommended)
-- Merging Staff and Visitor read policies
DROP POLICY IF EXISTS "Staff read meters" ON meters;
