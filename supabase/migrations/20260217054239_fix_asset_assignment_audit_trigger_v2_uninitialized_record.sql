...
BEGIN
    ...
    -- Determine action and get previous user info
    ...
    -- Use safe COALESCE and record field checks
    v_new_values := jsonb_build_object(
        ...
        'assigned_to_name', CASE WHEN NEW.assigned_to IS NOT NULL AND v_assigned_user.id IS NOT NULL 
            THEN CONCAT(v_assigned_user.first_name, ' ', v_assigned_user.last_name) ELSE NULL END,
        'assigned_to_employee_number', CASE WHEN NEW.assigned_to IS NOT NULL AND v_assigned_user.id IS NOT NULL THEN v_assigned_user.employee_number ELSE NULL END,
        ...
        'assigned_from_name', CASE WHEN NEW.assigned_from IS NOT NULL AND v_from_user.id IS NOT NULL 
            THEN CONCAT(v_from_user.first_name, ' ', v_from_user.last_name) ELSE NULL END,
        'assigned_from_employee_number', CASE WHEN NEW.assigned_from IS NOT NULL AND v_from_user.id IS NOT NULL THEN v_from_user.employee_number ELSE NULL END
    );
...
EXCEPTION
    WHEN OTHERS THEN
        -- Safely ignore errors to ensure main transaction continues
        -- But log to postgres console for debugging
