-- Add 'developer' role as an emergency override for the weekly report /
-- meeting document lock window, alongside 'super_admin'.
--
-- The existing has_role() helper does not know about the 'developer' role,
-- so we query public.profiles directly to check for either bypass role.

create or replace function public.weekly_report_can_mutate(p_week integer, p_year integer)
returns boolean
language plpgsql
stable
security definer
set search_path = public, pg_temp
as $$
declare
  v_unlock_until timestamp with time zone;
  v_deadline timestamp with time zone;
  v_role text;
begin
  -- Super admins and developers are the emergency override.
  select role::text into v_role
  from public.profiles
  where id = auth.uid();

  if v_role in ('super_admin', 'developer') then
    return true;
  end if;

  select wrmw.unlock_until
  into v_unlock_until
  from public.weekly_report_meeting_windows wrmw
  where wrmw.week_number = p_week
    and wrmw.year = p_year
  limit 1;

  if v_unlock_until is not null and now() <= v_unlock_until then
    return true;
  end if;

  v_deadline := public.weekly_report_lock_deadline(p_week, p_year);
  return now() <= v_deadline;
end;
$$;

create or replace function public.weekly_report_lock_state(p_week integer, p_year integer)
returns table (
  meeting_date date,
  grace_hours integer,
  unlock_until timestamp with time zone,
  lock_deadline timestamp with time zone,
  is_locked boolean
)
language sql
stable
security definer
set search_path = public, pg_temp
as $$
  with cfg as (
    select
      public.weekly_report_effective_meeting_date(p_week, p_year) as meeting_date,
      coalesce(
        (
          select wrmw.grace_hours
          from public.weekly_report_meeting_windows wrmw
          where wrmw.week_number = p_week
            and wrmw.year = p_year
          limit 1
        ),
        24
      ) as grace_hours,
      (
        select wrmw.unlock_until
        from public.weekly_report_meeting_windows wrmw
        where wrmw.week_number = p_week
          and wrmw.year = p_year
        limit 1
      ) as unlock_until,
      public.weekly_report_lock_deadline(p_week, p_year) as lock_deadline,
      (
        select role::text
        from public.profiles
        where id = auth.uid()
      ) as current_role
  )
  select
    cfg.meeting_date,
    cfg.grace_hours,
    cfg.unlock_until,
    cfg.lock_deadline,
    not (
      cfg.current_role in ('super_admin', 'developer')
      or (cfg.unlock_until is not null and now() <= cfg.unlock_until)
      or now() <= cfg.lock_deadline
    ) as is_locked
  from cfg;
$$;
