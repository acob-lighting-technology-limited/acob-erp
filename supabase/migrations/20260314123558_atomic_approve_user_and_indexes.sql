...
COMMENT ON FUNCTION public.generate_next_employee_number(INT) IS
  'Atomically generates the next available employee number for a given year using an exclusive advisory lock. Safe for concurrent admin approvals.';
...
COMMENT ON FUNCTION public.complete_user_approval IS
  'Atomically upserts the employee profile and deletes the pending_users record in a single transaction. Called by the approve-user API after auth user creation.';
...
BEGIN
  IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema = 'public' AND table_name = 'assets') THEN
    ...
    EXECUTE 'CREATE TRIGGER trg_assets_set_updated_at BEFORE UPDATE ON public.assets FOR EACH ROW EXECUTE FUNCTION public.set_updated_at()';
  END IF;
END $$;
...
BEGIN
  IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema = 'public' AND table_name = 'leave_requests') THEN
    ...
    EXECUTE 'CREATE TRIGGER trg_leave_requests_set_updated_at BEFORE UPDATE ON public.leave_requests FOR EACH ROW EXECUTE FUNCTION public.set_updated_at()';
  END IF;
END $$;
...
BEGIN
  IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema = 'public' AND table_name = 'help_desk_tickets') THEN
    ...
    EXECUTE 'CREATE TRIGGER trg_help_desk_tickets_set_updated_at BEFORE UPDATE ON public.help_desk_tickets FOR EACH ROW EXECUTE FUNCTION public.set_updated_at()';
  END IF;
END $$;
...
-- leave_requests: user's own leave history (column is user_id, not creator_id)
DO $$
BEGIN
  IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema = 'public' AND table_name = 'leave_requests')
  AND EXISTS (SELECT 1 FROM information_schema.columns WHERE table_schema = 'public' AND table_name = 'leave_requests' AND column_name = 'user_id') THEN
    EXECUTE 'CREATE INDEX IF NOT EXISTS idx_leave_requests_user_id ON public.leave_requests (user_id)';
  END IF;
END $$;

-- help_desk_tickets: requester + created_at composite (requester index already exists, add composite)
DO $$
BEGIN
  IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema = 'public' AND table_name = 'help_desk_tickets') THEN
    EXECUTE 'CREATE INDEX IF NOT EXISTS idx_help_desk_tickets_requester_created ON public.help_desk_tickets (requester_id, created_at DESC)';
  END IF;
END $$;
...
BEGIN
  IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema = 'public' AND table_name = 'notification_queue') THEN
    EXECUTE 'CREATE INDEX IF NOT EXISTS idx_notification_queue_status_process ON public.notification_queue (status, process_after)';
  END IF;
END $$;
...
BEGIN
  IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema = 'public' AND table_name = 'audit_logs') THEN
    EXECUTE 'CREATE INDEX IF NOT EXISTS idx_audit_logs_user_created ON public.audit_logs (user_id, created_at DESC)';
  END IF;
END $$;
...
BEGIN
  IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema = 'public' AND table_name = 'asset_assignments') THEN
    EXECUTE 'CREATE UNIQUE INDEX IF NOT EXISTS idx_asset_assignments_one_current_per_asset ON public.asset_assignments (asset_id) WHERE is_current = TRUE';
  END IF;
END $$;;
