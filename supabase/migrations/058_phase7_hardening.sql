...
CREATE POLICY "Staff can delete contacts" ON crm_contacts 
FOR DELETE TO authenticated 
USING (has_role('staff') OR has_role('admin'));

DROP POLICY IF EXISTS "Users can update contacts" ON crm_contacts;
CREATE POLICY "Staff can update contacts" ON crm_contacts 
...
CREATE POLICY "Staff can delete opportunities" ON crm_opportunities 
FOR DELETE TO authenticated 
USING (has_role('staff') OR has_role('admin'));

DROP POLICY IF EXISTS "Users can update opportunities" ON crm_opportunities;
CREATE POLICY "Staff can update opportunities" ON crm_opportunities 
FOR UPDATE TO authenticated 
USING (has_role('staff') OR has_role('admin'));

DROP POLICY IF EXISTS "Users can view all opportunities" ON crm_opportunities;
CREATE POLICY "Staff can view opportunities" ON crm_opportunities 
FOR SELECT TO authenticated 
USING (has_role('staff') OR has_role('admin'));

-- Table: crm_pipelines
DROP POLICY IF EXISTS "Authenticated users can create pipelines" ON crm_pipelines;
CREATE POLICY "Staff can create pipelines" ON crm_pipelines 
