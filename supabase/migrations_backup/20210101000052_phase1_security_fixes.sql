...
CREATE POLICY "employee can view consumption data" ON consumption_data 
FOR SELECT TO authenticated 
USING (has_role('employee') OR has_role('visitor'));
...
CREATE POLICY "employee can view remote tasks" ON remote_tasks 
FOR SELECT TO authenticated 
