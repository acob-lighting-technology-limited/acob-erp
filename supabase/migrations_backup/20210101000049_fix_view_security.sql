...
BEGIN
    IF EXISTS (SELECT 1 FROM pg_tables WHERE schemaname = 'public' AND tablename = 'customers') THEN
        ...
        -- INSERT: Admins and employee can create customers
        DROP POLICY IF EXISTS "Customers insert policy" ON customers;
        CREATE POLICY "Customers insert policy" ON customers FOR INSERT TO authenticated WITH CHECK (has_role('admin') OR has_role('employee'));
        
        -- UPDATE: Admins and employee can update customers
        DROP POLICY IF EXISTS "Customers update policy" ON customers;
        CREATE POLICY "Customers update policy" ON customers FOR UPDATE TO authenticated USING (has_role('admin') OR has_role('employee'));
