...
CREATE POLICY "employee can create crm activities" ON crm_activities 
FOR INSERT TO authenticated 
...
DROP POLICY IF EXISTS "employee read token records" ON token_records;
DROP POLICY IF EXISTS "employee create token records" ON token_records;
CREATE POLICY "Token records management" ON token_records 
FOR ALL TO authenticated 
USING (has_role('admin') OR has_role('employee'))
WITH CHECK (has_role('admin') OR has_role('employee'));
