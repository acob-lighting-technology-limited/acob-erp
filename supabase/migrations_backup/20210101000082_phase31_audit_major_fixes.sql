...
-- =====================================================
DROP POLICY IF EXISTS "System and employee can create notifications" ON notifications;
CREATE POLICY "employee can create notifications" ON notifications 
FOR INSERT TO authenticated 
...
-- =====================================================
-- 5. EVENT NOTIFICATIONS (Refine employee Acknowledge)
-- =====================================================
DROP POLICY IF EXISTS "Events employee acknowledge" ON event_notifications;
CREATE POLICY "Events employee acknowledge" ON event_notifications 
FOR UPDATE TO authenticated 
USING (
    (SELECT has_role('employee')) AND 
    acknowledged = false
) 
WITH CHECK (
