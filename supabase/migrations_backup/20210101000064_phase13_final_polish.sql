...
DROP POLICY IF EXISTS "employee can view remote tasks" ON remote_tasks;

CREATE POLICY "Remote tasks select policy" ON remote_tasks FOR SELECT TO authenticated USING (has_role('employee') OR has_role('admin'));
